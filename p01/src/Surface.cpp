...
Surface::Surface( int w, int h, int bpp, int flags)
	: _w(w), _h(h)
{
	 std::cout << "Surface created with dimensions: " 
              << WIDTH << "x" << HEIGHT << "\n";
    // ADDED JOYSTICK
...
{
    // Ensure x0, y0, x1, y1 are within bounds initially
    if (x0 < 0) x0 = 0;
    if (x1 < 0) x1 = 0;
    if (y0 < 0) y0 = 0;
    if (y1 < 0) y1 = 0;
    ...
    while (true)
    {
        ...
        {
            std::cerr << "Skipping invalid pixel at (" << x0 << ", " << y0 << ")\n";
        }
